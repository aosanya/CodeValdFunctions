...
type Config struct {
	...
	// ArangoDatabase is the ArangoDB database name (default "codevaldelfunctions").
...
//
//	CODEVALDELFUNCTIONS_GRPC_PORT   gRPC listener port (required)
...
//	FN_ARANGO_DATABASE              ArangoDB database (default "codevaldelfunctions")
...
//	CODEVALDELFUNCTIONS_AGENCY_ID   agency scope for this instance
...
func Load() Config {
	port := serverutil.MustGetEnv("CODEVALDELFUNCTIONS_GRPC_PORT")
	return Config{
		GRPCPort:       port,
		...
		ArangoDatabase: serverutil.EnvOrDefault("FN_ARANGO_DATABASE", "codevaldelfunctions"),
