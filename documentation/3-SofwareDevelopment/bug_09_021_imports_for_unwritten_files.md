...
After the MVP-SF-003 run, `lib/main.dart` on `main` imports:
...
Neither file is committed to the branch. `dashboard_notifier.dart` was in a `git.file.write` event (see BUG-09-020) but didn't make it to disk.
