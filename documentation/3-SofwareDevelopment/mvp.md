...
|---|---|---|---|
| FEAT-20260602-001 | `start-pipeline` function — fires on `work.pipeline.requested`, mints a `WorkflowRun`, publishes `work.pipeline.started` (confirmation) + `work.next.requested` (handoff to `next-task-selector`); the entry point for every orchestrated pipeline | 📋 Not Started | FEAT-20260602-001 in CodeValdWork (`WorkflowRun.name`) |
